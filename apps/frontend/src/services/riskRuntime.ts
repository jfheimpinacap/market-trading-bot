...
export function runRiskRuntimeReview(payload?: { triggered_by?: string }) {
  return requestJson<RiskRuntimeRun>('/api/risk-agent/run-runtime-review/', {
    method: 'POST',
...
export function getRiskRuntimeCandidates(params?: { run_id?: number; provider?: string; category?: string }) {
  ...
  return requestJson<RiskRuntimeCandidate[]>(`/api/risk-agent/runtime-candidates/${query.toString() ? `?${query}` : ''}`);
}
...
export function getRiskApprovalDecisions(params?: { run_id?: number; status?: string }) {
  ...
  return requestJson<RiskApprovalDecision[]>(`/api/risk-agent/approval-decisions/${query.toString() ? `?${query}` : ''}`);
}
...
export function getRiskRuntimeRecommendations(params?: { run_id?: number; recommendation_type?: string }) {
  ...
  return requestJson<RiskRuntimeRecommendation[]>(`/api/risk-agent/runtime-recommendations/${query.toString() ? `?${query}` : ''}`);
}

export function getRiskRuntimeSummary() {
  return requestJson<RiskRuntimeSummary>('/api/risk-agent/runtime-summary/');
}
