...
def _upsert_adjustment(*, adjustment_type: str, scope_type: str, scope_key: str, magnitude: Decimal, reason: str, metadata: dict):
    LearningAdjustment.objects.update_or_create(
        adjustment_type=adjustment_type,
        scope_type=scope_type,
        scope_key=scope_key,
        defaults={
            'is_active': magnitude != Decimal('0.0000'),
            'magnitude': magnitude,
            'reason': reason,
            'metadata': metadata,
        },
...
def rebuild_active_adjustments() -> dict[str, int]:
    ...
    return {'adjustments_processed': created_or_updated}
