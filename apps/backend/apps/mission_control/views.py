...
from apps.mission_control.serializers import (
    ...
    LivePaperTrialTrendDigestSerializer,
...
from apps.mission_control.governance_backlog_pressure.services.run import (
...
class LivePaperValidationDigestView(APIView):
    def get(self, request, *args, **kwargs):
...
class LivePaperTrialTrendView(APIView):
    def get(self, request, *args, **kwargs):
        serializer = LivePaperTrialTrendQuerySerializer(data=request.query_params)
        serializer.is_valid(raise_exception=True)
        payload = serializer.validated_data
        digest = build_live_paper_trial_trend_digest(
            limit=payload.get('limit', 5),
            preset=payload.get('preset'),
        )
        return Response(LivePaperTrialTrendDigestSerializer(digest).data, status=status.HTTP_200_OK)
...
class SessionResumeRecordListView(generics.ListAPIView):
...
class ApplySessionResumeView(APIView):
    def post(self, request, decision_id: int, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        applied_mode = serializer.validated_data.get('applied_mode', AutonomousResumeApplyMode.MANUAL_RESUME)
        automatic = applied_mode == AutonomousResumeApplyMode.AUTO_SAFE_RESUME
        result = apply_session_resume(decision=decision, automatic=automatic)
        payload = {
            'decision': AutonomousResumeDecisionSerializer(result.decision).data,
...
class GovernanceBacklogPressureSnapshotListView(generics.ListAPIView):
    ...
    queryset = GovernanceBacklogPressureSnapshot.objects.select_related(
...
class GovernanceBacklogPressureDecisionListView(generics.ListAPIView):
    ...
    queryset = GovernanceBacklogPressureDecision.objects.select_related(
        'linked_backlog_pressure_run', 'linked_backlog_pressure_snapshot',
    ).order_by('-created_at', '-id')[:400]
...
class GovernanceBacklogPressureRecommendationListView(generics.ListAPIView):
    ...
    queryset = GovernanceBacklogPressureRecommendation.objects.select_related(
        'linked_backlog_pressure_decision', 'linked_backlog_pressure_snapshot',
    ).order_by('-created_at', '-id')[:400]
